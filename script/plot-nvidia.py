import pandas as pd
import numpy as np
import seaborn as sns
import os.path
import matplotlib.pyplot as plt
import getopt
import sys

def genX(df : pd.DataFrame, name : str):
    x=np.asarray(df[name].to_numpy().nonzero()).transpose()
    return x


def nonzero(df : pd.DataFrame, name : str):
    df2 = df[df[name] != 0]
    return df2[name]


def genFig(df : pd.DataFrame, ax, title : str):
    num_rows=len(df.index)
    x=np.arange(num_rows)

    if nonzero(df,"faiss_warp").size==0 and nonzero(df,"faiss_block").size==0 and nonzero(df,"raft_radix_11bits_extra_pass").size==0 and nonzero(df,"grid_select").size==0:
        return None, None
    
    ax.plot(genX(df,"faiss_warp"), nonzero(df,"faiss_warp"),'4-',label="WarpSelect")  
    ax.plot(genX(df,"faiss_block"),nonzero(df,"faiss_block"),'^-',label="BlockSelect")   
    ax.plot(genX(df,"raft_radix_11bits_extra_pass"), nonzero(df,"raft_radix_11bits_extra_pass"),'8-',color="olivedrab",label="AIR Top-K")  
    ax.plot(genX(df,"grid_select"),nonzero(df,"grid_select"),'2-',label="GridSelect")

    plt.sca(ax)
    plt.xticks(x,df["k_power"].to_numpy())  #,rotation = 'vertical'
    plt.yscale('log')
    plt.title(title)

    handles,labels = ax.get_legend_handles_labels()
    return handles, labels

def drawFig(input_csv, output_png):
    csv=input_csv
    png_all_name=output_png

    print(csv, png_all_name)

    data= pd.read_csv(csv)
    count=0
    fig, axes = plt.subplots(nrows=4, ncols=4,figsize=(16,12))
    fig.text(0.0, 0.63, "Running Time (unit:ms)", va='center', rotation='vertical',fontsize=14)
    fig.text(0.55, 0.01, r'$log_{2}(K)$', ha='center',fontsize=14)

    data = data.loc[data['k_power'] >= 5]
    data = data.loc[data['k_power'] <= 11]

    for dist in [" Uniform"]:
        data_dist=data.query("dist == @dist")
        for bs in [1, 10, 100, 1000]:
            data_bs=data_dist.query("batch == @bs")
            for n_power in [10, 15,20,25]:
                N=pow(2,n_power)
                data_N=data_bs.query("N == @N")
                print(dist, bs, N, data_N.size)
                if data_N.size!=0:               
                    if dist[1:9] != "Unfriend":
                        title = "("+str(count)+") Batch="+str(bs)+" N="+(r'$2^{%s}$' %(str(n_power)))+" "+dist[1:9] #(r'$2^{%s}$' %(str(n_power)))
                    else:    
                        dist_name="Radix-adversarial"             
                        title = "("+str(count)+") Batch="+str(bs)+" N="+(r'$2^{%s}$' %(str(n_power)))+" \n "+dist_name #(r'$2^{%s}$' %(str(n_power)))                  
                    if count==0:
                        handles,labels = genFig(data_N,axes.flat[count],title)
                    else:
                        genFig(data_N,axes.flat[count],title)
                    count=count+1

    leg=fig.legend(handles, labels,ncol=5,bbox_to_anchor=(0.8,1),fontsize='large')
    plt.subplots_adjust(left=0.03, right=0.99, top=0.92, bottom=0.07, wspace=0.15, hspace=0.26)

    plt.savefig(png_all_name)



if __name__ == "__main__":
    opts, args = getopt.getopt(sys.argv[1:], "i:o:")
    input_csv=""
    output_png=""
    for op, value in opts:
        if op == "-i":
            input_csv = value
        elif op == "-o":
            output_png = value
    print(input_csv,output_png)
    drawFig(input_csv, output_png)
